...
func (oc *DefaultNetworkController) removeRemoteZonePod(pod *kapi.Pod) error {
	...
	// Remote the pod ips from the namespace address set.  Before that check if its a completed pod and
	...
	if util.PodCompleted(pod) {
		...
		if collidingPod != nil {
			klog.Infof("Will not release IP address: %s for %s/%s from namespace address set. Detected another pod"+
				" using this IP: %s/%s", util.JoinIPNetIPs(podIfAddrs, " "), pod.Namespace, pod.Name, collidingPod.Namespace, collidingPod.Name)
			klog.Infof("Will not release IP address: %s for %s from remote pod's namespace address set. Detected another pod"+
