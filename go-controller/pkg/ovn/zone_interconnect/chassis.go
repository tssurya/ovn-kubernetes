...
func (zch *ZoneChassisHandler) AddLocalZoneNode(node *corev1.Node) error {
	...
	if err := libovsdbops.UpdateChassisToLocal(zch.sbClient, node.Name, chassisId); err != nil {
		return fmt.Errorf("failed to update remote chassis into local %s, error: %w", node.Name, err)
	}
...
func (zch *ZoneChassisHandler) DeleteRemoteZoneNode(node *corev1.Node) error {
	...
	return libovsdbops.DeleteRemoteChassis(zch.sbClient, node.Name, chassisId)
}
