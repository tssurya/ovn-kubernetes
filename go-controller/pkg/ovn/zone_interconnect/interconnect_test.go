...
func createTransitSwitchPortBindings(sbClient libovsdbclient.Client, netName string, nodes ...*v1.Node) error {
	for _, node := range nodes {
		...
		err := libovsdbops.DebugCreateOrUpdatePortBinding(sbClient, &pb)
		if err != nil {
			return err
		}
	}
...
func checkInterconnectResources(zone string, netName string, nbClient libovsdbclient.Client, testNodesRouteInfo map[string]map[string]string, nodes ...*v1.Node) error {
