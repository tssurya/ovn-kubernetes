...
// CleanupRemoteChassis delete the chassis entry if its remote.
func CleanupRemoteChassis(sbClient libovsdbclient.Client, hostname, name string) {
	chassis := &sbdb.Chassis{
		Hostname: hostname,
		Name:     name,
	}
	...
	if strings.ToLower(chassis.OtherConfig["is-remote"]) == "true" {
		_ = DeleteChassis(sbClient, chassis)
	}
}
