...
func newZoneClusterController(ovnClient *util.OVNClusterManagerClientset, wf *factory.WatchFactory) (*zoneClusterController, error) {
	...
	idAllocator, err := NewIDAllocator("NodeIDs", maxNodeIDs+1)
	if err != nil {
		return nil, fmt.Errorf("failed to create an IdAllocator for the nodes, err: %w", err)
	}

	// Reserve the ids 0 and 1. We don't want to assign these ids to any of the nodes.
	// Since node id is used to derive node IP addresses we don't want to allocate any .1 IP
	if err := idAllocator.ReserveID("zero", 0); err != nil {
		return nil, fmt.Errorf("idAllocator failed to reserve id 0")
	}
	if err := idAllocator.ReserveID("one", 1); err != nil {
...
func (zcc *zoneClusterController) updateNodeAnnotationWithRetry(nodeName string,
	...
	resultErr := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if err != nil {
			return fmt.Errorf("unable to fetch nodes from watcher, err: %v", err)
		}
