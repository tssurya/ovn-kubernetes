...
func (eIPC *egressIPController) Start() error {
	...
	if eIPC.egressNodeHandler, err = eIPC.WatchEgressNodes(); err != nil {
		return fmt.Errorf("unable to watch egress nodes %v", err)
	}
...
func (h *egressIPClusterControllerEventHandler) DeleteResource(obj, cachedObj interface{}) error {
	...
	case factory.EgressNodeType:
		node := obj.(*kapi.Node)
		if err := h.eIPC.deleteNodeForEgress(node); err != nil {
			return err
		}
		nodeEgressLabel := util.GetNodeEgressLabel()
		nodeLabels := node.GetLabels()
...
func (h *egressIPClusterControllerEventHandler) SyncFunc(objs []interface{}) error {
	...
	if h.syncFunc != nil {
		...
		switch h.objType {
		case factory.EgressNodeType:
			syncFunc = h.eIPC.initClusterEgressPolicies
		case factory.EgressIPType:
			syncFunc = nil
...
func (eIPC *egressIPController) initClusterEgressPolicies(nodes []interface{}) error {
...
func (eIPC *egressIPController) deleteNodeForEgress(node *v1.Node) error {
	eIPC.allocator.Lock()
...
func (eIPC *egressIPController) validateEgressIPSpec(name string, egressIPs []string) (sets.String, error) {
...
func (eIPC *egressIPController) reconcileCloudPrivateIPConfig(old, new *ocpcloudnetworkapi.CloudPrivateIPConfig) error {
	...
	if shouldDelete {
		...
		// DONE IN LOCAL MASTER
		/*if err := eIPC.deleteEgressIPAssignments(egressIPName, []egressipv1.EgressIPStatusItem{statusItem}); err != nil {
			return err
		}*/
