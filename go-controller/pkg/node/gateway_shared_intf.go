...
func commonFlows(bridge *bridgeConfiguration)  ([]string, error) {
	...
	if config.IPv4Mode {
		...
		/*dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=105, in_port=%s, ip, pkt_mark=0x3f0, ct_state=+new "+
				"actions=ct(commit, zone=%d, nat(src=%s), exec(set_field:%s->ct_mark)),output:%s", defaultOpenFlowCookie, ofPortPatch, config.Default.ConntrackZone, physicalIP.IP, ctMarkOVN, ofPortPhys))
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=1002, in_port=%s, ip, pkt_mark=0x3f0 "+
				"actions=load:%s->nw_src,output:%s", defaultOpenFlowCookie, ofPortPatch, hexVal, ofPortPhys))*/
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=105, in_port=%s, ip, pkt_mark=0x3f0 "+
				"actions=ct(commit, zone=%d, nat(src=%s), exec(set_field:%s->ct_mark)),output:%s", defaultOpenFlowCookie, ofPortPatch, config.Default.ConntrackZone, physicalIP.IP, ctMarkOVN, ofPortPhys))
		/*dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=10, in_port=%s, ip"+
				"actions=ct(commit, zone=%d,nat)", defaultOpenFlowCookie, ofPortPhys, config.Default.ConntrackZone))*/
...
func newSharedGateway(nodeName string, subnets []*net.IPNet, gwNextHops []net.IP, gwIntf, egressGWIntf string,
	...
	for _, hostSubnet := range subnets {
		// shared gateway mode uses mp0 as path for egress traffic from
		// egressIP pods to other nodes or host-net pods in the cluster
		...
		err := initSharedGatewayNATRules(cidrNet)
		if err != nil {
