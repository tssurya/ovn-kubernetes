...
import { GRAPH_AREA_DRAGGING_EVENT, GRAPH_AREA_SELECTED_EVENT, isGraph } from '../types';
...
export const useAreaSelection = (modifiers = ['ctrlKey']) => {
    ...
    const areaSelectionRef = useCallbackRef((node) => {
        if (node) {
            // TODO fix any type
            ...
            const drag = d3
                ...
                const { offsetX, offsetY } = event.sourceEvent instanceof MouseEvent ? event.sourceEvent : { offsetX: 0, offsetY: 0 };
                const { width: maxX, height: maxY } = elementRef.current.getDimensions();
                setDraggingState((prev) => (Object.assign(Object.assign({}, prev), { areaSelectDragEnd: new Point(Math.min(Math.max(offsetX, 0), maxX), Math.min(Math.max(offsetY, 0), maxY)) })));
            }))
                .on('end', action(() => {
...
export const withAreaSelection = (modifier = ['ctrlKey']) => (WrappedComponent) => {
    const Component = (props) => {
        const areaSelectionProps = useAreaSelection(modifier);
        return _jsx(WrappedComponent, Object.assign({}, props, areaSelectionProps));
    };
