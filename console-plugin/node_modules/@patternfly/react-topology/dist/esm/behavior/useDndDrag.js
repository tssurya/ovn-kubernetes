...
export const useDndDrag = (spec, props) => {
    ...
    const propsRef = useRef(props != null ? props : EMPTY_PROPS);
    ...
    const createKeyHandlerId = useCallback((event = '') => `${event}.useDndDrag-${monitor.getHandlerId()}`, [monitor]);
    useEffect(() => () => {
        if (canUseDOM) {
    ...
    []);
    const refCallback = useCallbackRef(useCallback((node) => {
        let ownerDocument;
        if (node) {
    ...
    const collected = useMemo(() => computed(() => (spec.collect ? spec.collect(monitor, propsRef.current) : {}), {
...
export const withDndDrag = (spec) => (WrappedComponent) => {
    const Component = (props) => {
        ...
        return _jsx(WrappedComponent, Object.assign({}, props, dndDragProps, { dndDragRef: dndDragRef }));
    };
