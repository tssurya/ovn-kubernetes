...
const TaskNodeInner = observer((_a) => {
    ...
    useAnchor(useCallback((node) => new TaskNodeSourceAnchor(node, detailsLevel, statusIconSize + 4, verticalLayout), 
    ...
    useAnchor(useCallback((node) => new TaskNodeTargetAnchor(node, whenSize + whenOffset, detailsLevel, statusIconSize + 4, verticalLayout), 
    // Include scaleNode to cause an update when it changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    const taskPill = (_jsx(TaskPill, Object.assign({ x: verticalLayout ? 0 : whenOffset, y: 0, className: css('pf-topology__pipelines__task-node', className), scaleNode: scaleNode, hover: hovered, statusIconSize: statusIconSize, pillRef: pillRef, width: width, taskRef: taskRef, element: element }, rest)));
    return (_jsx("g", Object.assign({ className: css('pf-topology__pipelines__task-node', className), transform: `${scaleNode ? `translate(${translateX}, ${translateY})` : ''} scale(${nodeScale})`, ref: hoverRef }, { children: !toolTip || disableTooltip ? (taskPill) : (_jsx(Tooltip, Object.assign({ triggerRef: taskRef, position: "bottom", enableFlip: false }, (toolTipProps !== null && toolTipProps !== void 0 ? toolTipProps : {}), { content: toolTip }, { children: taskPill }))) })));
});
const TaskNode = (_a) => {
